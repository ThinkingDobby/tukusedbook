...
class BookDetailActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        try {
            ...
            if (admin) {
                ...
                bookDetail_icon_like.setOnClickListener {
                    ...
                    if (like) {
                        like = false
                        val tmp =
                            FirebaseDatabase.getInstance().getReference("Book").child(book.book_id)
                                .child("like_users")
                                .child(nowId)
                        ...
                        likeRef.get().addOnSuccessListener {
                            cnt = it.value as Long
                            likeRef.setValue(cnt - 1)
                            bookDetail_tv_like.text = (cnt - 1).toString()
                        }
            ...
            Firebase.database.getReference("User").child(book.seller_id).get()
            ...
            Toast.makeText(this, "삭제된 글이에요.", Toast.LENGTH_SHORT).show()
        }
    }
